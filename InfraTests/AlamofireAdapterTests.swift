...
class AlamofireAdaptar: HttpPostClient {
...
extension AlamofireAdapterTests {
    func makeSut(file: StaticString = #file, line: UInt = #line) -> AlamofireAdaptar {
    ...
    func testRequestFor(url: URL = makeUrl(), data: Data?, action: @escaping (URLRequest) -> Void) {
        ...
        UrlProtocolStub.obeserverRequest { request = $0 }
    ...
    func expectResult(_ expectedResult: Result<Data?, HttpError>, when stub : (data: Data?, response: HTTPURLResponse?, error: Error?), file: StaticString = #file, line: UInt = #line) {
        ...
        sut.post(to: makeUrl(), with: makeValidData()) { receivedResult in
            ...
            default: XCTFail("Expected \(expectedResult) got \(receivedResult) insted", file: file, line: line)
            }
            exp.fulfill()
        }
        wait(for: [exp], timeout: 1)
